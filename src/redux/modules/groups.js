...
export function handleAndFetchGroups(accessToken, uid, groupIds) {
  return async function(dispatch) {
    try {
      dispatch(fetchingGroups())
      const groups = await getGroups(accessToken, groupIds)
      dispatch(fetchingGroupsSuccess(groupsObjectFromArray(uid, groups)))
    } catch (error) {
